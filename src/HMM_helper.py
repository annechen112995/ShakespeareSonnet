...
from dictionaries import import_syllables
...
def generate_poem(hmm, obs_map, n_words=500):
    # Get reverse map.
    ...
    # Get dictionary of syllables
    syllables = import_syllables()
...
def animate_emission(hmm, obs_map, M=8, height=12, width=12, delay=1):
    ...
    for i, wordcloud in enumerate(wordclouds):
        ...
        ax.imshow(
            wordcloud.to_array(),
            extent=(x - r, x + r, y - r, y + r), aspect='auto', zorder=-1)
    ...
    def animate(i):
        if i >= delay:
            ...
            # Set text.
            text.set_text(' '.join(
                [obs_map_r[e] for e in emission][:i + 1]).capitalize())
